	...
	with model.sess as sess:
		...
		while True:
			...
			while ( (time.time() - wall_start) / 60 ) <= args.evaluation_minutes:
				...
				_, loss = sess.run([model.train_op, model.loss], feed_dict={input_handle_ph: training_handle, model.dropout: args.dropout, model.lr: model.args.lr})
				print("train_step: ", train_step, "loss: ", loss)
				total_train_loss += loss

			args.eval_cnt += 1
			#print("train loss: ", total_train_loss)
			#print("model final_scores: ", model.final_scores)
