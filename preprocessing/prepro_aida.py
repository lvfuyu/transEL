...
def process_aida(in_filepath, out_filepath):
    ...
    ent_id_changes = 0
    with open(in_filepath, encoding = 'utf-8') as fin, open(out_filepath, "w",encoding ='utf-8' ) as fout:
...
def split_dev_test(in_filepath):
    with open(in_filepath,encoding = 'utf-8') as fin, open(args.output_folder+"temp_aida_dev", "w",encoding = 'utf-8') as fdev,\
            open(args.output_folder+"temp_aida_test", "w", encoding = 'utf-8') as ftest:
        fout = fdev
        for line in fin:
